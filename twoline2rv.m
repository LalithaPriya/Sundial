...
%     // input start stop times manually
     if ((typerun ~= 'v') && (typerun ~= 'c'))
         % ------------- enter start/stop ymd hms values --------------------
           if (typeinput == 'e')
               datev = julian2UTC(jdate);
               startyear = datev(1); %input('input start year');
               startmon  = datev(2); %input('input start mon');
               startday  = datev(3);%input('input start day');
               starthr   = datev(4);%input('input start hr');
               startmin  = datev(5);%input('input start min');
               startsec  = datev(6);%input('input start sec');
               jdstart = jday( startyear,startmon,startday,starthr,startmin,startsec );

               stopyear = 2018;%input('input stop year');
               stopmon  = 03; %input('input stop mon');
               stopday  = 25;%input('input stop day');
               stophr   = 0;%input('input stop hr');
               stopmin  = 0;%input('input stop min');
               stopsec  = 0;%input('input stop sec');
               ...
               deltamin = 1;%input('input time step in minutes ');
           end;
